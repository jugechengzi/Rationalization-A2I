...
def train_two_head(model, optimizer, opt_adv, dataset, device, args,writer_epoch):
    '''
    正常generator和adv generator共享encoder
    :param model:
    :param optimizer:
    :param opt_adv:
    :param dataset:
    ...
    for (batch, (inputs, masks, labels)) in enumerate(dataset):
        ...
        #update the generator and the predictor

        # freeze the adv head's param
        name_adv=[]
        ...
        #use hinge loss for the adv head
        cross_entropy_adv= F.cross_entropy(pred_logits_adv, labels_adv,reduction='none')     #目前定的是离相反类别的距离，pred应该是让它越远越好(即entropy大)，后续可以试一下距离本类别的距离，pred让他越近越好
        temp=torch.ones_like(pred_logits_adv)*0.5   #全0.5的预测
        ...
        #update the adv generator head
        optimizer.zero_grad()
        opt_adv.zero_grad()

        # unfreeze the adv head's param
        for idx, p in model.gen_fc_adv.named_parameters():
            if idx in name_adv:
                p.requires_grad = True

        #freeze the generator and the predictor
        name_gen_pred=[]
        ...
        # unfreeze the generator and the predictor
        for i in model.gen_pred:
            for name, p in i.named_parameters():
...
def train_separate_two_head(rnp_model,adv_gen, optimizer, opt_adv, dataset, device, args,writer_epoch):
    '''
    normal generator和adv generator不共享encoder
    :param rnp_model:
    :param adv_gen:
    :param optimizer:
    :param opt_adv:
    ...
    for (batch, (inputs, masks, labels)) in enumerate(dataset):
        ...
        if args.use_hinge==1:
            ...
            #use 0.5 as both the positive and negative labels for the adv head
...
def train_postattack(rnp_model,adv_gen, optimizer, opt_adv, dataset, device, args,writer_epoch):
    '''
    normal generator和adv generator不共享encoder
    将训练好的rnp(包括用了对抗训练的)固定，单独训练attacker
    :param rnp_model:
    :param adv_gen:
    :param optimizer:
    :param opt_adv:
...
def train_separate_two_head_normal_generator(rnp_model,adv_gen, opt_pred,opt_gen, opt_adv, dataset, device, args,writer_epoch):
    '''
    normal generator和adv generator不共享encoder
    训练generator和predictor,同时附带更新attacker,但是此时attacker不影响主回路的更新
    :param rnp_model:
    :param adv_gen:
    :param optimizer:
    :param opt_adv:
...
def train_separate_two_head_attacker_predictor(rnp_model,adv_gen, opt_pred,opt_gen, opt_adv, dataset, device, args,writer_epoch):
    '''
    固定住normal generator 和attacker
    predictor由attacker给的东西来训练
    :param rnp_model:
    :param adv_gen:
    :param optimizer:
    :param opt_adv:
    ...
    for (batch, (inputs, masks, labels)) in enumerate(dataset):
        ...
        #update the generator,but freeze the predictor
        opt_pred.zero_grad()
        opt_gen.zero_grad()
        opt_adv.zero_grad()
...
def validate_acc_on_valset(model, dev_loader, device):
    """
    验证在验证集上的预测准确率（非rationale质量），写于2023.09.06，two_head_adv版本
    :param model:
    :param annotation_loader:
    :param device:
    :return:
    """
    TP = 0
...
def validate_separate_acc_on_valset(rnp_model,adv_gen, dev_loader, device):
    """
    验证在验证集上的预测准确率（非rationale质量），rnp和adv generator分别采用两个模型，不再集成到一起
    :param model:
    :param annotation_loader:
    :param device:
    :return:
